...
public class GetUsersMeRestController {
    ...
    public ResponseEntity<GetUsersMeResponseDto> getUsersMe() {
        ...
        User user = useCase.getUserProfile(new GetUserProfileUseCase.Param(currentUserId));
        return new ResponseEntity<>(toResponseDto(user), HttpStatus.OK);
    }

    private GetUsersMeResponseDto toResponseDto(User user) {
        return new GetUsersMeResponseDto(user.getId(), user.getDisplayName(), user.getPicture());
    }
}
