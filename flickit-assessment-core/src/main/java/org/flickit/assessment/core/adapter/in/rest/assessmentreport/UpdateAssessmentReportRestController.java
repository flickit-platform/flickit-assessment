...
public class UpdateAssessmentReportRestController {
    ...
    public ResponseEntity<Void> updateAssessmentReportVisibility(@PathVariable UUID assessmentId,
                                                                 @RequestBody UpdateAssessmentReportVisibilityRequestDto request){
        UUID currentUserId = userContext.getUser().id();
        useCase.updateReportVisibility(toParam(assessmentId, request.visibility(), currentUserId));
