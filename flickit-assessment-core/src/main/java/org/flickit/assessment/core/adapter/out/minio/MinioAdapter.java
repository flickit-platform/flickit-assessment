...
public class MinioAdapter implements CreateFileDownloadLinkPort, UploadEvidenceAttachmentPort {
    ...
    @SneakyThrows
    @Override
    public String uploadAttachment(MultipartFile attachment, UUID uniqueFileName) {
        ...
        if (attachment.getOriginalFilename() != null)
            extension = attachment.getOriginalFilename().substring(attachment.getOriginalFilename().indexOf(DOT));

        String objectName = uniqueDir + uniqueFileName.toString() + extension;
        writeFile(bucketName, objectName, attachment.getInputStream(), attachment.getContentType());
