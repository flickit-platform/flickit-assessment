...
public interface LoadQuestionsAnswerDashboardPort {
    ...
    record Result(List<Answer> answers, List<Evidence> evidences, Integer totalQuestion, Integer totalEvidences) {

        public record Answer(long id, int confidence) {
        }

        public record Evidence(UUID id, Integer type, Boolean resolved) {
        }
    }
}
