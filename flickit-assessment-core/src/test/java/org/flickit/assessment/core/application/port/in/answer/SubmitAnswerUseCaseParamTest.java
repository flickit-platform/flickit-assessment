...
class SubmitAnswerUseCaseParamTest {
    ...
    void testSubmitAnswer_assessmentIdIsNull_ErrorMessage() {
        var throwable = assertThrows(ConstraintViolationException.class,
            () -> new SubmitAnswerUseCase.Param(null, 1L, 1L, 1L, ConfidenceLevel.getDefault().getId(), Boolean.FALSE));
    ...
    void testSubmitAnswer_questionnaireIdIsNull_ErrorMessage() {
        ...
        var throwable = assertThrows(ConstraintViolationException.class,
            () -> new SubmitAnswerUseCase.Param(assessmentResult, null, 1L, 1L, ConfidenceLevel.getDefault().getId(), Boolean.FALSE));
    ...
    void testSubmitAnswer_questionIdIsNull_ErrorMessage() {
        var assessmentResult = UUID.randomUUID();
        var throwable = assertThrows(ConstraintViolationException.class,
            () -> new SubmitAnswerUseCase.Param(assessmentResult, 1L, null, 1L, ConfidenceLevel.getDefault().getId(), Boolean.FALSE));
