...
public interface CreateAdviceNarrationUseCase {

    void createAdviceNarration(Param param);

    @Value
    @EqualsAndHashCode(callSuper = false)
    class Param extends SelfValidating<Param> {

        @NotNull(message = CREATE_ADVICE_NARRATION_ASSESSMENT_ID_NOT_NULL)
        UUID assessmentId;

        @NotNull(message = CREATE_ADVICE_NARRATION_ADVICE_LIST_ITEMS_NOT_NULL)
        List<AdviceListItem> adviceListItems;

        @NotNull(message = CREATE_ADVICE_NARRATION_ATTRIBUTE_LEVEL_TARGETS_NOT_NULL )
