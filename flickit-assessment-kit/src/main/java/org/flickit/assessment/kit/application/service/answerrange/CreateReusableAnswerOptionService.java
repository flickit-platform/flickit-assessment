...
public class CreateReusableAnswerOptionService implements CreateReusableAnswerOptionUseCase {
    ...
    @Override
    public Result createReusableAnswerOption(Param param) {
        ...
        if (!answerRange.isReusable())
            throw new ValidationException(CREATE_ANSWER_OPTION_ANSWER_RANGE_NON_REUSABLE);

        long answerOptionId = createAnswerOptionPort.persist(toCreateParam(param));
        return new CreateReusableAnswerOptionUseCase.Result(answerOptionId);
    }

    private void checkUserAccess(CreateReusableAnswerOptionUseCase.Param param) {
